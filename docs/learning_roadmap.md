...
**Bối cảnh:** CLI pipeline: parse arguments → đọc JSONL → label → ghi output → in summary. `_build_parser` tạo argparse, `_run_cli` chạy async logic, `main` gọi `asyncio.run` và xử lý KeyboardInterrupt.
...
| 5 | 7 | Split dataset (hash-based deterministic split) |
